...
async function migrateServerUrl() {
    ...
    const {serverUrl, groupId} = await browser.storage.local.get(["serverUrl", "groupId"]) as {
        serverUrl: string,
        groupId: string
    };
...
browser.runtime.onInstalled.addListener(async (details) => {
    ...
    if (details.reason === "update" && details.previousVersion && details.previousVersion < "2.0.0") {
