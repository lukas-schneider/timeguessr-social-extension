...
import { AIGuessInfoModal } from "./AIGuessInfoModal";
...
function createAIGuessInfoModal(data: RoundResultEnhancedBreakdownData) {
  try {
...
function createEnhancedMap(
  data: RoundResultEnhancedBreakdownData | FinalResultEnhancedBreakdownData,
) {
  try {
...
function enhanceBreakdownList(
  ...
  enhancedMap: EnhancedMap | null,
) {
  let imageDetails = createImageDetails(data);
  tgRoundNames
    .map(
      (roundName) => document.getElementById("block" + capitalize(roundName))!,
    )
    .map(clone)
    .filter(Boolean)
    .forEach((elem, index) => {
